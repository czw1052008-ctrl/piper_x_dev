...
class RefineRunner:
    ...
    def _init_ros(self) -> None:
        ...
        self._node.create_subscription(
            String, '/reach/fsm_state', self._on_fsm_state, 10)

        # Subscribe to FSM status (human-readable).
        self._node.create_subscription(
    ...
    def run_one(self, loop_idx: int) -> bool:
        print(f'\n{"─"*60}')
        print(f'Loop {loop_idx} — resetting FSM to IDLE …')
        ...
        # 2. Restore entry pose (arm back to ALIGN exit position).
        ...
        state = self._wait_for_state({_STATE_REFINING} | _TERMINAL_STATES, timeout_s=8.0)
